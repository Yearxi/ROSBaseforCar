...
int main(int argc, char **argv) {
  ...
  node->declare_parameter<std::string>("device_ip_difop", std::string("192.168.1.102"));
  node->declare_parameter<std::string>("group_ip", "224.1.1.2");
  node->declare_parameter<int>("difop_port", 2369);
  node->declare_parameter<int>("UDP_PORT_NUMBER", 2368);
  node->get_parameter("device_ip", devip_str_);
  node->get_parameter("device_ip_difop", devip_str_difop);
  node->get_parameter("group_ip", group_ip);
  node->get_parameter("difop_port", UDP_PORT_NUMBER_DIFOP);
  node->get_parameter("UDP_PORT_NUMBER", UDP_PORT_NUMBER);

  ldlidar::LiPkg* lidar_pkg = new ldlidar::LiPkg();
  ldlidar::CmdNetInterfaceLinux* cmd_port = new ldlidar::CmdNetInterfaceLinux(devip_str_,UDP_PORT_NUMBER_DIFOP,devip_str_difop,UDP_PORT_NUMBER,group_ip,UDP_PORT_NUMBER);
